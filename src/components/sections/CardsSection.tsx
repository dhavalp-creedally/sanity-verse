...
export default function CardsSection({section}: {section: any}) {
    return (
        <div className={`cards-section py-16 lg:py-20 bg-gradient-to-t from-slate-100 to-slate-50`}>
            <div className="container">
                ...
                <div className="card-items grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {section?.cardsLists && section.cardsLists.map((cardsList: any, index: number) => {
                        ...
                        return  (
                            <div key={index} className={`feature-item flex flex-col shadow-md items-center p-12 rounded-xl bg-white justify-center ${cardsList?.class || ""}`}>
